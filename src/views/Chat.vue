    ...
    <v-card>
      ...
      <v-card-actions>
        <v-dialog v-model="dialog" width="500">
          <template v-slot:activator="{ on }">
            <v-btn
              text
              v-on="on"
              @click="dialogShow('video')"
              :disabled="inputTypes.indexOf('video') == -1"
            >
            ...
            <v-btn
              text
              v-on="on"
              @click="dialogShow('audio')"
              :disabled="inputTypes.indexOf('audio') == -1"
            >
            ...
            <v-btn
              text
              v-on="on"
              @click="dialogShow('image')"
              :disabled="
                inputTypes.indexOf('fullImage') == -1 &&
                inputTypes.indexOf('image') == -1
              "
            >
          ...
          <v-card raised>
            ...
            <v-container>
              <VideoCard
                v-show="dialogType == 'video'"
                v-on:childContent="getMedia"
              ></VideoCard>
              <AudioCard
              ...
              ></AudioCard>
              <PhotoCard
                v-show="dialogType == 'image'"
                v-on:childContent="getMedia"
              ></PhotoCard>
              <CamCard
                v-show="dialogType == 'camera'"
                v-on:childContent="getMedia"
              ></CamCard>
            </v-container>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="success" text @click="sendMsg(dialogType)"
                >Send</v-btn
              >
            </v-card-actions>
          </v-card>
        ...
        <v-text-field
          ...
          label="Typing here"
          @change="textChange"
          :disabled="inputTypes.indexOf('text') == -1"
        ></v-text-field>
        <v-btn id="textBtn" color="green" @click="sendMsg('text')" dark text
          >send</v-btn
        >
      </v-card-actions>
    </v-card>
    ...
    handleResponse(res) {
      ...
      for (let itemKey in res.data) {
        ...
        for (let typeKey in itemData) {
          if (typeKey == "pcm") {
            type = typeKey;
            payload = this.convertPCMToWAV(
              base64.toByteArray(itemData[typeKey].data)
            );
